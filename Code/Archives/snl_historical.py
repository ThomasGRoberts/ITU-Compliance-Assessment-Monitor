...
Input:			n/a
Output:			../Data/Historical Analysis/[YYYYMMDD]/networks_historical_[YYYYMMDD].csv	
"""
...
# Check whether today's subdirectory exists in the relevant Daily Analysis sub-directory
...
if not CHECK_FOLDER:
	...
	os.system('python3 snl_namechange_download.py')
	print("The SNL's name-change data has been downloaded.")
# Now check for today's subdirectory in the Historical Analysis directory
MYDIR = ('../Data/Historical Analysis/' + assessmentdate)
CHECK_FOLDER = os.path.isdir(MYDIR)
# If it doesn't exist, then create it and run the scrape scripts.
if not CHECK_FOLDER:
	os.makedirs(MYDIR)
# Add another subdirectory inside that one to house the historical nearby shortlists
MYDIR = ('../Data/Historical Analysis/' + assessmentdate + '/Historical Nearby Shortlists')
CHECK_FOLDER = os.path.isdir(MYDIR)
# If it doesn't exist, then create it and run the scrape scripts.
if not CHECK_FOLDER:
	os.makedirs(MYDIR)
# Add another subdirectory to house the compliance assessment results
...
## Import the list of newtorks that have had their names changed
newnames_list = []
oldnames_list = []
with open('../Data/Daily Analysis/' + assessmentdate + '/SNL Archives/snl_namechange_' + assessmentdate + '.csv') as f:
	reader = csv.reader(f, delimiter=",")
	for row in reader:
		if row[0] != 'New Name':
...
for i in np.arange(len(df_licenses)):
	print('Fetching license information for', df_licenses.at[i, 'Network Name'], '...')
...
df_licenses.to_csv('../Data/Historical Analysis/' + assessmentdate + '/networks_historical_' + assessmentdate + '.csv', index = None)
